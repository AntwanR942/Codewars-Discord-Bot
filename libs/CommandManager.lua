...
function Command:__init(Name, Handle, Parent)
    self._Name = Name
    self._Handle = Handle
    self._IsSubCommand = (Parent ~= nil and true or false)
    if self._IsSubCommand then
        if not SubCommands[Parent._Name] then
            SubCommands[Parent._Name] = {}
        end

        SubCommands[Parent._Name][self._Name] = self
    else
...
function Command:AddSubCommand(Name, Handle)
    return Command(Name, Handle, self)
end
...
function Command:GetInfo(Info)
    return (self._Info ~= nil and self._Info or nil)
end
